...
**Sprint 3 记录的设计缺口就此关闭** —— 流量域从此有离线源，可参与批流对账。

```text
✅ 归档作业 16/16 自检全过，RESULT=OK
     [archive] Kafka 实际消息数 = 20000
     [archive] 解析完成，有效行数 = 20000
     [info]    归档覆盖 703 个 dt 分区

✅ 湖仓侧实证（查表，不是看命令返回）
     ods_behavior_event 行数 = 20000
     dt 分区数             = 703
     事件时间范围          = 2024-10-02 21:53:33 → 2026-09-26 13:16:13

✅ 零消息丢失的硬证据
     Kafka latest offset 合计 = 20000  ==  归档行数 20000

✅ 漏斗逐级收窄（与 AGENTS.md 8.2 的断言一致）
     VIEW 10472 > CLICK 5759 > CART 2095 > BUY 628
     （FAVORITE 1046 为旁支，符合"显著少于 VIEW"）

✅ 实时链路自动恢复
     [ OK ] 实时链路已恢复并通过健康检查
```

**排掉的六层障碍**（全部有确凿证据，无一靠猜）：

| 层 | 障碍 | 根因 |
| --- | --- | --- |
| 1 | 阶段静默空转、0.6 秒报成功 | 漏加 `case` 分支 + 无兜底 |
| 2 | `Failed to find data source: kafka` | 官方 Spark 镜像不带 Kafka 连接器 |
| 3 | "构建成功"却没变化 | Dockerfile 改在了会被同步覆盖的副本 |
| 4 | `ClassNotFoundException`（executor 侧） | **重建镜像 ≠ 容器换镜像** |
| 5 | 读到 20000 却写 0 行 | 格式串里的 `T` 非法 → `event_time` 全 NULL → 被过滤 |
| 6 | 自检全绿但表是空的 | 空值断言打在空的目标表上，**空洞地通过** |

> **六层里有三层属于同一类：「成功信号」不可信。**
> 命令返回成功、构建报成功、自检全绿，都不能替代
> **回到目标状态去验证**（表里到底有多少行、镜像里到底有没有那个 jar）。

### 10.3 仍待完成

```text
⏳ 流量域 DWD / DWS / ADS 与逐窗口对账（把行为事件纳入对账范围）
⏳ scripts/verify-sprint-4.sh
⏳ 文档收口（README / AGENTS / DEVELOPMENT_LOG）
```

### 10.4 本 Sprint 实测发现并修复的 5 个系统性缺陷

> 与 10.2 的"六层障碍"互补：10.2 按**时间顺序**记录归档作业踩的坑，
> 这里按**缺陷类型**归纳其中可复用到其它模块的部分（都属"看起来已经好了"的类型）。
